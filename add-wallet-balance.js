...
async function addWalletBalance() {
  try {
    // Find the test user
    const testUser = await User.findOne({ _id: new mongoose.Types.ObjectId('68a2ffa2e50fb244ba4905dd') });
    if (!testUser) {
    ...
    if (!wallet) {
      ...
      await wallet.save();
      console.log('Created new wallet with balance 10000');
    } else {
      // Update existing wallet
      wallet.balance = 10000;
      await wallet.save();
      console.log('Updated wallet balance to 10000');
    }
